...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchTables = async () => {
      ...
      try {
        ...
        if (response.ok) {
          const tableResponse =
            (await response.json()) as OrderLookupResponse[];
          setTables(
    ...
    const fetchMenuItems = async () => {
      ...
      try {
        const response = await fetch(
          `${restUrl}rms/v1/menu-items?per_page=100`,
          {
            headers: { "X-WP-Nonce": nonce },
          },
        );
        ...
        if (response.ok) {
          const menuItemResponse =
            (await response.json()) as OrderMenuItemResponse[];
          setMenuItems(
            menuItemResponse.map((menuItem) => ({
              id: Number(menuItem.id),
              title: menuItem.title,
              price: Number(menuItem.price),
              menu_category: menuItem.menu_category ?? [],
              dietary_tag: menuItem.dietary_tag ?? [],
              prep_time_minutes: Number(menuItem.prep_time_minutes ?? 0),
              is_available: menuItem.is_available ?? false,
              image_url: menuItem.featured_image_url ?? null,
            })),
          );
        }
      } catch {
  ...
  const updateOrderStatus = async (status: OrderStatus) => {
    ...
    try {
      const response = await fetch(
        `${restUrl}rms/v1/orders/${selectedOrder.id}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <PageWrapper>
      ...
      >
        <Tabs.List mb="md">
          <Tabs.Tab value="today">Active Orders</Tabs.Tab>
        ...
        <Tabs.Panel value="today">
          {isLoading ? (
            ...
            <Stack gap="lg" maw={1440} mx="auto">
              ...
              {error ? (
                <Alert color="attention" title="Order update failed">
                  {error}
                </Alert>
              ) : null}
              ...
              <Paper p="sm" bg={theme.other.surfaceContainerLow}>
                ...
                <Text fw={600} mt={4}>
                  {selectedOrder.table_id
                    ? getTableName(selectedOrder.table_id)
                    : "Takeout"}
                </Text>
              </Paper>
            </Group>
